...
const eventInstructionLogRouter = (app, passport) => {
    app.get('/eventList/:eventId/eventInstructionLog/', passport.authenticate('jwt', { session: false }), (req, res) => {
        const { eventId } = req.params;
        pool.query(`SELECT *
                FROM event_instruction_log e_m_s 
                WHERE event=${eventId}
                ORDER BY date `, (error, result) => {
            if (error) {
                console.log(error);
    ...
    app.put(
        ...
        (req, res) => {
            ...
            pool.query(
                `INSERT INTO depart ( event, date, instruction) VALUES(${eventId},'${date}','${instruction}'`,
                (error, result) => {
                    if (error) {
                        console.log(error);
                        res.status(500).json({ success: false, message: error });
    ...
    app.post('/eventList/:eventId/eventInstructionLog/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
        const { id } = req.params;
        const { date,
    ...
    app.put('/eventList/:eventId/generateEventInstructionLog', passport.authenticate('jwt', { session: false }), (req, res) => {
        const { eventId } = req.params;
        const queries = [];
        queries.push(
            new Promise((resolve, reject) => {
                ...
                WHERE e.id=${eventId}`,
                    (error, result) => {
                        ...
                        const { event_start, ob_fio, event_name, st_fio, member_fio } = result[0]
                ...
                pool.query(
                    ...
                    (error, result) => {
                        ...
                        result.forEach(resultItem => {
                            let instruction = ''
                            const { type, mount_name, depart_name, rout_comp, rout_name, laba_name, program_names } = resultItem
                            if (type === 'Отдых') {
                                ...
                                instruction = `Разрешить отделению ${depart_name} выход на занятия ${program_names || ''} в ${laba_name}`
                            }
                            if (instruction) { insertValuesArray.push(`(${eventId} ,'${resultItem.start}', '${instruction}')`) }
                        })
        ...
        pool.query(`DELETE FROM event_instruction_log WHERE event =${eventId}`, (error, result) => {
            if (error) {
                console.log(error);
                res.status(500).json({ success: false, message: error });
                return
